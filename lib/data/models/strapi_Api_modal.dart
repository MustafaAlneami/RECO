...
//           'vidTitle': attributes['title'] ?? '',
//           'vidDescription': attributes['description'] ?? '',
//           'vidTime': attributes['time'] ?? '',
//           'vidDuration': attributes['duration'] ?? '',
//           'vidDate': attributes['date'] ?? '',
//           'vidPlatform': attributes['vidPlatform'] ?? '',
//           'vidLink': attributes['vidLink'] ?? '',
//           'vidThumbnail': thumbnailUrl,
//           'chanelName': attributes['chanelName'] ?? '',
//           'chanelLogo': attributes['chanelLogo'] ?? '',
//           'chanelsTags': attributes['chanelsTags'] ?? '',
//           'channelId': attributes['channelId'] ?? 0,
//         });
//       }
//       return videoCards;
//     } else {
...
class ApiService {
  ...
  // Unified fetch method that returns both models
  Future<Map<String, List<dynamic>>> fetchVideoCardsAndCalendarEvents() async {
    ...
    if (response.statusCode == 200) {
      ...
      final List<CalendarStrapiEvent> calendarEvents = [];
      ...
      for (var item in body['data']) {
        ...
        // Add to both models
        //videoCards.add(VideoCard.fromMap(mapData));
        calendarEvents.add(CalendarStrapiEvent.fromJson(mapData));
      }
